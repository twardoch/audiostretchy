...
class TestAudioStretch:
    ...
    def test_convert_to_int16_mono(self):
        ...
        # Float-to-int truncates toward zero: -0.5*32767 = -16383.5 → -16383
    ...
    def test_convert_to_int16_stereo(self):
        ...
        # -0.5*32767 = -16383.5 truncates toward zero → -16383
        expected = np.array([0, 32767, 16383, -32767, -16383, 0], dtype=np.int16)
        np.testing.assert_array_equal(int16_samples, expected)
    ...
    def test_convert_from_int16_mono(self):
        ...
        expected = np.array(
            [[0.0, 16383 / 32767, -16384 / 32767, 1.0, -1.0]], dtype=np.float32
        )
